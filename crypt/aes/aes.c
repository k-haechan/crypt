...
void KeyExpansion(const uint8_t *key, uint32_t *w, int Nk, int Nr) {
    uint32_t temp;
    int i = 0;
    while (i < Nk) {
...
static void AddRoundKey(uint8_t state[4][4], const uint32_t *w, int round) {
    for (int c = 0; c < 4; c++) {
...
static void ShiftRows(uint8_t state[4][4]) {
    uint8_t t = state[1][0]; state[1][0] = state[1][1]; state[1][1] = state[1][2]; state[1][2] = state[1][3]; state[1][3] = t;
...
void AES_Encrypt(const uint8_t *input, uint8_t *output, const uint8_t *key, int keySize) {
    int Nk = keySize / 32, Nr = Nk + 6;
    uint32_t w[60]; uint8_t state[4][4];
    KeyExpansion(key, w, Nk, Nr);
    for (int r = 0; r < 4; r++) for (int c = 0; c < 4; c++) state[r][c] = input[r + 4 * c];
    ...
    for (int r = 0; r < 4; r++) for (int c = 0; c < 4; c++) output[r + 4 * c] = state[r][c];
}
